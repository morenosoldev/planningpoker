use actix_web::{post, get, web, HttpResponse, Result, error::ErrorInternalServerError, HttpRequest, error::ErrorUnauthorized};
...
pub async fn create_room(
    ...
    let insert_result = collection.insert_one(&new_room, None).await
    ...
    let room_response = GameRoomResponse {
        ...
        participants: new_room.participants.iter().map(|id| ParticipantInfo {
            id: id.to_string(),
            username: "".to_string(),
            profile_image: None,
        }).collect(),
...
pub async fn join_room(
    ...
    let room_response = GameRoomResponse {
        ...
        participants: room.participants.iter().map(|id| ParticipantInfo {
            id: id.to_string(),
            username: "".to_string(),
            profile_image: None,
        }).collect(),
...
async fn get_participants_info(db: &Database, participant_ids: &[String]) -> Result<Vec<ParticipantInfo>, Box<dyn std::error::Error>> {
    let users_collection = db.collection::<User>("users");
    let mut participants_info = Vec::new();

    for id in participant_ids {
        if let Ok(object_id) = mongodb::bson::oid::ObjectId::parse_str(id) {
            if let Ok(Some(user)) = users_collection.find_one(doc! { "_id": object_id }, None).await {
...
pub async fn get_room(
    ...
    let object_id = mongodb::bson::oid::ObjectId::parse_str(room_id.as_str())
    ...
    let participants_info = get_participants_info(&db, &room.participants)
        .await
        .map_err(ErrorInternalServerError)?;
...
pub async fn handle_new_story(db: &Database, room_id: &str, mut story: Story) -> Result<(), Box<dyn std::error::Error>> {
...
pub async fn handle_vote(db: &Database, room_id: &str, user_id: &str, story_id: &str, value: i32) -> Result<(), Box<dyn std::error::Error>> {
    println!("handle_vote kaldt med room_id: {}, user_id: {}, story_id: {}, value: {}", room_id, user_id, story_id, value);
    let collection = db.collection::<GameRoom>("game_rooms");
    let users_collection = db.collection::<User>("users");
...
pub async fn handle_end_voting(db: &Database, room_id: &str, story_id: &str, final_score: i32) -> Result<(), Box<dyn std::error::Error>> {
    println!("handle_end_voting kaldt med room_id: {}, story_id: {}, final_score: {}", room_id, story_id, final_score);
    let collection = db.collection::<GameRoom>("game_rooms");
    ...
    let current_story = room.current_story
        .ok_or("Ingen aktiv historie fundet")?;
...
pub async fn handle_save_final_score(
    ...
    let story_obj = story_json.as_object()
        .ok_or("Story er ikke et gyldigt objekt")?;
    
    let title = story_obj.get("title")
        .and_then(|v| v.as_str())
        .ok_or("Manglende titel")?
        .to_string();
    
    let description = story_obj.get("description")
        .and_then(|v| v.as_str())
        .ok_or("Manglende beskrivelse")?
        .to_string();
    
    let votes = story_obj.get("votes")
        .and_then(|v| serde_json::from_value::<Vec<Vote>>(v.clone()).ok())
        .unwrap_or_default();
    
    let final_score = story_obj.get("final_score")
        .and_then(|v| v.as_i64())
        .map(|v| v as i32)
        .unwrap_or(0);
    ...
    println!("Historie gemt i completed_stories collection med id: {:?}", insert_result.inserted_id);
    ...
    if let Some(game_server) = GAME_SERVER.lock().unwrap().as_ref() {
        ...
        let websocket_story = serde_json::json!({
...
pub async fn get_completed_stories(
    ...
    let room = match rooms_collection
        .find_one(doc! { "_id": object_id }, None)
        .await
        .map_err(ErrorInternalServerError)?
    {
    ...
    let mut cursor = completed_stories_collection
        .find(doc! { "room_id": room_id.as_str() }, None)
        .await
        .map_err(ErrorInternalServerError)?;
